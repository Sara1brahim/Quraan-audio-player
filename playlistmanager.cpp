            ...
            case 8: {
                ...
                while (!exitPlayback) {
                    ...
                    case 80: // Down arrow
                        ...
                        break;
                    /*case 'p':  // Play next Playlist
                      {
                        int currentIndex = l.getindex(playlistName);

                        if (currentIndex == -1) {
                            cout << "Error: Current playlist not found." << endl;
                            break;
                        }

                        int nextIndex = (currentIndex + 1) % l.getSize();  // Loop to the first playlist if at the end

                        PlaylistInfo* nextPlaylist = l.getpos(nextIndex);
                        if (nextPlaylist) {
                            playlistName = nextPlaylist->getName();  // Update the current playlist name
                            selectedPlaylist = nextPlaylist;

                            cout << "Switching to next playlist: " << playlistName << endl;
                            selectedPlaylist->playSurah();  // Start playback of the new playlist
                        }
                        else {
                            cout << "Next playlist not found." << endl;
                        }
